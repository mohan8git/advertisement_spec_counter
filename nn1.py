...
def func(x,y,outputs_length,outputs_len_previous,faceBoxes,frame):
    try:
        global c1
        global c2
        global gender
...
def centre(top, right, bottom, left):
    ...
    return x, y
import asyncio, websockets, json, time,websocket

ws_url = "ws://3.130.4.197:8000/"                
ws=websocket.WebSocket()
ws.connect(ws_url)

async def command_receiver(outputs_length,face_locations_length,c1,c2,total_p,nv,c3,c4):
        async with websockets.connect(ws_url) as websocket:
                #print("websocket : ", websocket)		
                await websocket.send(json.dumps({'users_count': outputs_length,'current_users':face_locations_length,'male_users':c1,'female_users':c2,'total_users_count':total_p,'total_current_users':nv,'total_male_users':c3,'total_female_users':c4}))               
...
##                ws.send(json.dumps({'users_count': 0}))
            if (bbox_xywh is not None) & (frameCounter%1 ==0) :
                ...
                start = timer()
                if outputs_length <1:
                    ws.send(json.dumps({'users_count': 0,'current_users':0,'male_users':c1,'female_users':c2}))#'total_users_count':total_p,'total_current_users':nv,'total_male_users':c3,'total_female_users':c4}))
                                ...
                                if flag == True:
                                    if e not in nv:
                                       # print("navu print karyu")
                                        nv.append(e)
                                        gen = func(temp1[j][0],temp1[j][1],outputs_length,outputs_len_previous,faceBoxes,frame)
##                                        #print("Answer",gender)
                                        time.sleep(1)
                                        if gen=='Male':
                                            c3 = c3+1
            ...
            if cv2.waitKey(1) & 0xFF == ord('q'):
                ...
                ws.send(json.dumps({'users_count': outputs_length,'current_users':face_locations_length,'male_users':c1,'female_users':c2,'total_users_count':len(total_p),'total_current_users':len(nv),'total_male_users':c3,'total_female_users':c4}))
